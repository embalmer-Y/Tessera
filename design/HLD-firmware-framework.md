...
- **V1 明确不做**（裁剪清单）：逻辑节点（多立方体并联）实现——仅命名空间与接口预留〔DEC-26〕；真机四板移植（M3 后按 DEC-15 另列）；真机在环测试；**APP 业务状态持久化**（升级/回滚后状态丢弃，DR-15）；**审计留痕落盘**（V1 = 内存环形 + sys:get-audit 导出，掉电丢失，DR-07）；输入遥测为轮询变化上报（无硬件中断驱动，DR-02）。
...
> 裁决批次 1（2026-09-20）：Q-03/04/05/08/09/12 → DEC-19…24；裁决批次 2（2026-09-21）：Q-06/Q-07 → DEC-25/26（正文相关值已同步 DEC 语义）；仍待裁：**Q-10、Q-11**。
