...
- **V1 明确不做**（裁剪清单）：逻辑节点（多立方体并联）实现——仅命名空间与接口预留〔DEC-26〕；真机三板移植（ESP32-S3/P4/H7，DEC-28 后；M3 后按 DEC-15 另列）；真机在环测试；**APP 业务状态持久化**（升级/回滚后状态丢弃，DR-15）；**审计留痕落盘**（V1 = 内存环形 + sys:get-audit 导出，掉电丢失，DR-07）；输入遥测为轮询变化上报（无硬件中断驱动，DR-02）。
...
- 包格式〔DEC-21〕：`wasm 模块 + manifest(CBOR) + COSE-Sign1(ed25519) 签名`单文件容器；manifest 含版本、最低框架版本、能力清单。
...
### 4.6 内存预算（DEC-29：每板动态分配计算；默认值 M2 实测按行修订）

**板卡策略（DEC-28）**：框架面向未来、优先高性能高配置板卡；资源不足者放弃——**RP2350 已移出目标集**（Zephyr 无 PSRAM 驱动 + 520KB SRAM）。目标板 = **ESP32-S3 / ESP32-P4 / STM32H7 + native_sim（CI）**。

**分配规则（构建期按板生成预算表，非全局常量）**：

1. 内部 SRAM 固定优先：框架安全静态（ts-safety/ts-core 表与审计 ≈24KB）+ Zephyr/网络栈/mbedTLS ≈96KB + zenoh ≈32KB + WAMR 运行时 ≈24KB；
2. 剩余内部 SRAM → 系统堆 + 余量（**余量 ≥15% 内部 SRAM**）；
3. 外部 RAM（有则）：承载 WAMR 实例堆（DEC-27 每板动态配置）。

**每板默认（提案；M2 实测校准）**：

| 板 | 内部 SRAM | 外部 RAM | WAMR 实例堆默认 | APP 并发默认 |
|---|---|---|---|---|
...
- v0.2.2 · 2026-09-21：owner 问询——§3.4 补 APP 线程模型（Q-13 提案：单线程/禁自建/三层限制）。
