...
- DEC-22（WDT 周期）、DEC-27（队列深度/订阅数/优先级）已裁；无未决。
...
- v0.2.2 · 2026-09-22：M1 实现收敛留痕——① §2"每步执行前后发布"收敛为**完成后单条**（失败步亦观测，golden 稳定）；② §5 estop 事后补发的机制 = 巡检项检测 forced 上升沿后发布（ISR 内不提交工作项——estop 调用图队列禁令的落点），补发时延上界 = 巡检周期。
