...
| **M2b** | ts-hal 权限（ts_perm_v1）+ WAMR 宿主 + 样例 APP | **M2b 本地全绿（2026-09-23）+ 补审查 IR-05…20 处置（同日）**；M2b.2 进行中——环境批交付（2026-09-26，twister 11/11〔47 用例〕）；**Q-23 实证批交付（同日，framework.wamrdemo 4 用例）**：B 方案饿死实证（105-113ms 全量转嫁）/TOCTOU 窗口 203ns/A 通路可用——数据支持方案 A（修订：锁收口随接线批顺带）；**接线批待 owner 裁 Q-23** | HLD §7 |
