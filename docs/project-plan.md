...
| **M2b** | ts-hal 权限（ts_perm_v1）+ WAMR 宿主 + 样例 APP | **M2b 本地全绿（2026-09-23）+ 补审查 IR-05…20 处置（同日）**；M2b.2 进行中——环境批 + Q-23 实证批交付（2026-09-26）；**DEC-43 实现批交付（同日）：锁收口（write_lock/commit_locked/迁移路径/预算原子化/pubq 互斥）+ framework.conc 并发回归——twister 13/13（54 用例）全绿**；余 = 接线批第二单元（appmgr 执行线程 + ts_* natives + 租约挂钩） | HLD §7 |
