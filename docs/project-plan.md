...
| **M3a** | ts-net（zenoh-pico；发现/key/sys 命令——A06 对齐依赖） | **M3a.1 本地全绿（2026-09-23）**：传输缝 + keyspace/pubq/session/linkmon + zenoh-pico 1.10.1 钉版接线（真实绑定编译链接绿）；M3a.2（遥测/sys 命令面/L3 端到端）待续 | HLD §7 |
